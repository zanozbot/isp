...
################
### SSH
### (2) Allow outgoing SSH connections, deny incoming syn packets
...
for ipAddress in $MY_ISP; do
  iptables -A INPUT -p icmp --icmp-type echo-request -d $ipAddress -j ACCEPT
done
